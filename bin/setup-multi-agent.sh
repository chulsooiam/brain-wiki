#!/usr/bin/env bash
# brain-wiki: multi-agent skill installer
...
#   - Codex CLI      : symlink to ~/.codex/skills/brain-wiki
#   - OpenCode       : symlink to ~/.opencode/skills/brain-wiki
#   - Gemini CLI     : symlink to ~/.gemini/skills/brain-wiki
...
if [ ! -d "$SKILLS_DIR" ]; then
  echo "ERROR: $SKILLS_DIR does not exist. Are you running this from the brain-wiki repo?"
  exit 1
fi
...
echo "brain-wiki: multi-agent skill installer"
echo "Repo: $REPO_ROOT"
echo

# Codex CLI
link_if_missing "$SKILLS_DIR" "$HOME/.codex/skills/brain-wiki" "Codex CLI"

# OpenCode
link_if_missing "$SKILLS_DIR" "$HOME/.opencode/skills/brain-wiki" "OpenCode"

# Gemini CLI
link_if_missing "$SKILLS_DIR" "$HOME/.gemini/skills/brain-wiki" "Gemini CLI"
...
echo "  - Codex CLI:   codex --list-skills | grep brain-wiki"
