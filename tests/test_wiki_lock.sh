...
# MSYS/Git Bash strips CR bytes from subprocess argv when stdout is a pipe
# (command substitution), so on such hosts the CR never reaches the script —
# probe first and skip rather than mis-fail. The validation itself is covered:
# invoked with a real CR (direct terminal), the script rejects with exit 4.
CR_PROBE=$(bash -c 'printf %s "$1"' _ $'a\rb')
if [ "$CR_PROBE" = $'a\rb' ]; then
  RC_CR=$( (wl acquire $'wiki/concepts/Foo\rbar.md' >/dev/null 2>&1); echo $? )
  assert_eq "acquire carriage-return path rejected" "4" "$RC_CR"
else
  echo "SKIP acquire carriage-return path rejected (platform strips CR from piped argv)"
fi
