...
# ─── retrieve.py CLI: default-on self-heal contract ───────────────────────────
def _make_sandbox(tmpdir, with_wiki_page=False):
    """Minimal vault layout with the retrieval scripts copied in."""
    import shutil
    sandbox = Path(tmpdir)
    ...
    if with_wiki_page:
        (sandbox / "wiki").mkdir()
        (sandbox / "wiki" / "sample.md").write_text(
            "# Sample Page\n\nA retrievable fact: the flux capacitor "
            "needs 1.21 gigawatts.\n", encoding="utf-8")
    return sandbox


def test_retrieve_exits_10_without_pages():
    """With no index AND no wiki pages, retrieve.py has nothing to build → 10."""
    with tempfile.TemporaryDirectory() as tmpdir:
        sandbox = _make_sandbox(tmpdir, with_wiki_page=False)
        result = subprocess.run(
            [sys.executable, str(sandbox / "scripts" / "retrieve.py"), "test query"],
            capture_output=True, text=True, timeout=30,
        )
        assert_eq("retrieve.py exit 10 with nothing to index", 10, result.returncode)
        assert_true("retrieve.py prints friendly error",
                    "no wiki pages" in result.stderr,
                    hint=result.stderr[:200])


def test_retrieve_no_auto_build_flag_refuses():
    """--no-auto-build restores the old refusal even when pages exist."""
    with tempfile.TemporaryDirectory() as tmpdir:
        sandbox = _make_sandbox(tmpdir, with_wiki_page=True)
        result = subprocess.run(
            [sys.executable, str(sandbox / "scripts" / "retrieve.py"),
             "test query", "--no-auto-build"],
            capture_output=True, text=True, timeout=30,
        )
        assert_eq("--no-auto-build exits 10 on missing index", 10, result.returncode)
        assert_true("--no-auto-build names itself in the error",
                    "no-auto-build" in result.stderr,
                    hint=result.stderr[:200])


def test_retrieve_auto_builds_inline():
    """Default-on: missing index + existing pages → build inline, then answer."""
    with tempfile.TemporaryDirectory() as tmpdir:
        sandbox = _make_sandbox(tmpdir, with_wiki_page=True)
        env = dict(os.environ, PYTHONUTF8="1")
        result = subprocess.run(
            [sys.executable, str(sandbox / "scripts" / "retrieve.py"),
             "flux capacitor gigawatts", "--no-rerank"],
            capture_output=True, text=True, timeout=60, env=env,
        )
        assert_eq("auto-build then answer exits 0", 0, result.returncode)
        assert_true("auto-build note on stderr",
                    "auto-build:" in result.stderr, hint=result.stderr[:300])
        data = json.loads(result.stdout)
        assert_true("auto-built index finds the page",
                    any("sample" in (c.get("page_path") or "")
...
def main():
    ...
    test_retrieve_exits_10_without_pages()
    test_retrieve_no_auto_build_flag_refuses()
    test_retrieve_auto_builds_inline()
