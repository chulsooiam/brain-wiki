---
name: wiki-retrieve
description: "Hybrid retrieval primitive for the Compound Vault. Replaces the v1.6 static hot→index→drill read order with contextual-prefix + BM25 + cosine-rerank, modeled on Anthropic's Sept 2024 Contextual Retrieval research (35-49-67% retrieval-failure reduction). Default-on since v2.0 — retrieve.py self-heals a missing/stale index inline (synthetic prefixes, zero egress); `bin/setup-retrieve.sh` remains for manual provisioning and the egress consent flow. Feature-detected by wiki-query and autoresearch. Triggers on: retrieve, hybrid retrieval, BM25, rerank, contextual retrieval, search the chunks, chunk search, vault search, semantic search, what chunks match, find relevant passages."
allowed-tools: Read Bash
---

# wiki-retrieve: Hybrid Retrieval over the Vault

The v1.6 query path was `Read(hot.md) → Read(index.md) → Read(3-5 pages) → synthesize`. It worked, but page-level granularity loses to chunk-level granularity any time the answer lives in a specific passage rather than a whole page. The v1.7 `wiki-retrieve` skill is the chunk-level upgrade — default-on since v2.0 (the retriever provisions itself inline from existing wiki pages, synthetic prefixes only), and it replaces nothing when there is nothing to index.

**Origin**: This skill is original to brain-wiki. There is no upstream kepano equivalent. The technique is from [Anthropic's Sept 2024 Contextual Retrieval research](https://www.anthropic.com/news/contextual-retrieval) — we implement it as agent-skill plumbing.
...
If `retrieve.py` exits 10 (no wiki pages to index yet, the inline auto-build failed, or `--no-auto-build` was passed), `wiki-query` falls back to the legacy v1.6 `Read(index.md) → Read(N pages)` order. No user-visible breakage.
...
- DragonScale addresses: [`skills/wiki-ingest/SKILL.md`](../wiki-ingest/SKILL.md) §Address Assignment (the `DragonScale Memory` concept page is created per-vault, not shipped)
