...
11. **Version lineage**. Deterministic — run `python3 scripts/lineage-check.py check`. See the **Version Lineage** section below.
12. **Contradiction audit**. Grep all `> [!contradiction]` callouts across `wiki/` and cross-check `wiki/meta/contradictions.md` (register format: wiki-ingest §Contradictions): callouts missing from the register → MEDIUM (add rows); register rows still `open` whose pages no longer carry callouts → MEDIUM (stale — resolve or restore); rows `open` longer than ~30 days → LOW (surface for a decision, contradictions are meant to be worked off, not accumulated). Skip entirely if neither callouts nor a register exist.
13. **Glossary stub orphans — expected, not findings**. Pages with `glossary_seed: true` linked only from `glossary.md` are pre-positioned link targets (skills/glossary-seed), NOT orphans. Exclude them from check 1; flag only seeded stubs that some source page has linked but nobody expanded (still stub-length despite inbound links) — those are ingestion debt, LOW.

**Emit actionable findings into the work queue** (the reground loop — a finding buried in a dated report is where maintenance goes to die). After writing the lint report, queue every finding that an ingest session can actually fix:

```bash
python3 scripts/work-queue.py add --kind stale --page "Page Name" --detail "claim X outdated by [[New Source]]"
```

Kind mapping: stale claims → `stale`; lineage findings (check 11) → `lineage`; aging open contradictions (check 12) → `contradiction`; linked-but-unexpanded stubs (check 13) → `stub-expand`. `add` is idempotent per (kind, page) — re-running lint refreshes details without duplicating. Orphans, dead links, and frontmatter gaps are fixed during lint itself, not queued. Ingest drains the queue per wiki-ingest §Work Queue.

---

## Defect Log & Meta Loop

Lint finds problems in *pages*; the defect log finds problems in the *system that writes them*. When an ingest produces a bad page — wrong entity split, hallucinated claim, mangled table, a summary that missed the document's actual point — record the defect, don't just fix the page:

`wiki/meta/defect-log.md` (create on first use):

```markdown
| date | skill | page | defect | root-cause tag |
|---|---|---|---|---|
| 2026-08-07 | wiki-ingest | [[Policy Rev1]] | summary missed annex B entirely | long-doc-skim |
| 2026-08-09 | transcript-distill | [[Weekly 2026-08-08]] | action item attributed to wrong speaker | speaker-inference |
```

Root-cause tags are free-form but reused deliberately — the tag is what makes mining possible.

**The meta rule**: during each lint run, group defect rows by root-cause tag. When **three or more defects share a tag**, the authoring rules — not the pages — are the problem. Draft a guideline amendment as a proposal note in `wiki/meta/proposals/` (what recurring failure, which SKILL.md section, exact proposed wording, evidence rows), and queue it: `work-queue.py add --kind defect --page "<proposal note>" --detail "3+ defects tagged X"`.

**The loop proposes; it never self-adopts.** No SKILL.md, template, or convention changes without the operator explicitly approving the proposal. On approval, apply the edit, mark the proposal note `status: adopted`, and annotate the defect rows it closes. This is how the toolkit tunes itself to an institution's corpus instead of staying tuned to its authors'.

---

## Version Lineage

Documents that replace each other — policy revisions, framework editions, template versions — declare it in frontmatter:

```yaml
supersedes: "[[Data Governance Policy (2017)]]"      # single link or list
superseded_by: "[[Data Governance Policy (Rev 2)]]"  # absent on the current head
```

The relationship is **symmetric by convention**: if A supersedes B, B must declare `superseded_by: "[[A]]"`. A page with no `superseded_by` is the chain's head — the current version.

Run the deterministic checker (read-only, exit 1 when findings exist):

```bash
python3 scripts/lineage-check.py check            # findings with suggested fixes
python3 scripts/lineage-check.py chains           # every chain, current head first
```

Severity mapping for the lint report:

| finding | severity | action |
|---|---|---|
| `cycle` | HIGH | supersession loop — no version is current; resolve with the user, never auto-fix |
| `dangling` | HIGH | lineage points at a nonexistent page — either create the target (a legacy edition worth a stub) or drop the field |
| `asymmetry` | MEDIUM | auto-fixable — the checker prints the exact missing field; apply it with the user's blanket approval, one lock per page |

Two follow-on checks that need judgment (fold into **Stale claims**):

- A **superseded page** should open with a `> [!stale]` callout pointing to its successor (`> Superseded by [[New Page]] — kept for history.`). Flag superseded pages without one.
- Pages that **link to a superseded page** in a present-tense claim ("the policy requires…") probably mean the chain head. Flag; do not rewrite silently.
