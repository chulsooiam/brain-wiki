...
4. If this is a fresh vault (no `wiki/hot.md`), offer the guided quick start: read the `quickstart` skill and follow its gate question. An accepted quickstart replaces step 5 — its interview and build take over from here. A declined one costs one keystroke and continues below. An unfinished one (the skill's status check reports IN_PROGRESS) is offered as a resume.
