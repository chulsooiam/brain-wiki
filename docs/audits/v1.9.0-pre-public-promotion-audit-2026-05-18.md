...
**Date:** 2026-05-18
**Audit scope:** publish-readiness across 5 dimensions before promoting v1.7-v1.9 work from `chulsooiam/brain-wiki` (private) to `chulsooiam/brain-wiki` (public).
...
- **Files M4** — Possible duplicate basename between `wiki/sources/brain-wiki-ecosystem-research.md` and `wiki/entities/Ar9av-obsidian-wiki.md`. Verify intentional source→entity flow.
...
3. **References F1–F5 closed iff:** `rg -n 'chulsooiam/brain-wiki' docs/install-guide.md CONTRIBUTING.md AGENTS.md GEMINI.md ATTRIBUTION.md .claude-plugin/*.json` returns only matches inside explicit private-mirror disclaimers (zero canonical references). Plus `rg -n 'raw.githubusercontent.com/AI-Marketing-Hub' docs/` returns empty.
...
# F1-F5 cascade
rg -q 'chulsooiam/brain-wiki' .claude-plugin/plugin.json .claude-plugin/marketplace.json
...
Public promotion to `chulsooiam/brain-wiki` remains pending explicit user "go" per standing local-until-explicit-go rule. The post-fix state is shippable; the gate is consent, not quality.
