...
| 5 | Adoption friction (GUI onramp, one-liner installer) | **PARTIAL** | CLI transport reduces friction; GUI onramp deferred to v2.5+; no `npx brain-wiki init` shipped |
...
| `chulsooiam/brain-wiki` (us) | 4.1k★ | 4.1k★ | flat | local-only branch | v1.6.0 |
...
**Copilot #2257 (Obsidian CLI integration)** — Still OPEN. Last update 2026-03-06 (3 months stale). 0 comments. **brain-wiki v1.7 §3.2 shipped exactly what this issue describes.** Genuine competitive moat: we shipped what Copilot has been planning for 3+ months.
...
**Implication for brain-wiki's #1 verdict:** The derivative-outputs gap (compass artifact Gap #3 + backlog items #5, #9, #14) is **WIDER** than the May-16 compass artifact captured. NotebookLM now ships 4 first-class artifact types (Audio, Video, Mind Maps, Reports) plus Study Guides, Briefs, Quizzes, Data Tables. v1.7 ships zero. The deferral of `wiki-derive` to v2.0 was correct as a sequencing call, but the competitive gap is now larger and the v2.0 spec should consider adding Video Overviews (Marp + TTS pipeline) given NotebookLM's new bar.
...
- **LOW (L10 - market)**: Copilot CLI integration issue #2257 has been stale for 3 months. Genuine competitive moat for brain-wiki on the CLI-native axis. Worth surfacing in the positioning narrative ("the only Claude+Obsidian stack that's actually CLI-native today").
