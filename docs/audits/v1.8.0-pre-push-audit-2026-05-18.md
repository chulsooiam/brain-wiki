# v1.8.0 Pre-Push Audit — brain-wiki
...
- Install command in CLAUDE.md (`claude plugin marketplace add chulsooiam/brain-wiki`) consistent with `source.repo` in marketplace.json — MATCH
...
### Install rehearsal
- `claude plugin marketplace add chulsooiam/brain-wiki` → marketplace.json references this as a `github` source with `ref: main`. **Caveat:** repo not yet pushed to GitHub. Install command currently fails for fresh users. This is intentional (local until explicit go), not a finding.
- `claude plugin install brain-wiki` → standard. No issues anticipated.
