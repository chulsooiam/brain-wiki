...
All notable changes to brain-wiki. Format: [Keep a Changelog](https://keepachangelog.com/en/1.1.0/). Versioning: [SemVer](https://semver.org/).
...
- **Backlog-sweep pattern** (`transcript-distill` §Content propagation): adopting the propagation rule after a backfill leaves every already-distilled entry unpropagated. Documented two-phase sweep — extract candidates as structured data (agents propose, edit nothing; validate page names and heading anchors at the barrier), then apply **grouped by target page** so each page has exactly one writer and that writer sees all its candidates at once, which is the only way to catch a March candidate superseded by a June one. Phase-2 agents must re-judge against the live page; extraction agents work from entries alone and cannot know what a page already says.
...
The professional-corpus release: **corpus tier + conversion pipeline + three-query surface + default-on hybrid retrieval**, plus the professional-corpus skill track. Capabilities for wikis built from real institutional document collections (meeting recordings, policy revisions, survey forms, dense acronym spaces) rather than clean articles. Motivated by and validated against a UN-agency data-governance corpus (~400 mixed-format documents, 187 ASR meeting transcripts).

### Added

- **Corpus retrieval tier (`.sources/` → `.vault-meta/corpus/`)** — a second retrieval tier over primary source documents, parallel to the curated wiki. Source documents converted to Markdown under `.sources/` are chunked and indexed separately by `scripts/corpus-index.py` (+ `corpus-dedup.py`, `corpus-bm25.py`, `corpus-retrieve.py`). Every chunk carries a `tier` label derived from its top-level folder (a folder name containing "Tier N" → `N`; anything else → the slugified folder name); optional `.vault-meta/corpus-tiers.json` overrides the mapping and configures a per-tier ranking bonus. Ranking gives authoritative tiers a small bonus, so a governance-framework chunk outranks a meeting aside at equal lexical score.
- **The rebind pattern** — corpus scripts do not fork the retrieval stack: they import `bm25-index.py` / `rerank.py` as modules and rebind their state paths onto the corpus store, so every stock ranking improvement propagates to the corpus tier automatically. Contract documented in `docs/corpus-query.md`; guarded by a rebind-contract tripwire test.
- **Three-query surface** — `/wiki-query` (conclusions tier), `/corpus-query` (sources tier), `/combined-query` (both tiers). `/combined-query` answers in a fixed shape — Answer / Wiki says / Sources say / Discrepancies — with per-tier attribution; scores are never interleaved across tiers. Skills live in `.claude/skills/` (corpus-query, combined-query, corpus-convert) with matching project-scope commands in `.claude/commands/`.
- **Conversion pipeline** — `scripts/convert.py` (driver: single-file + resumable batch with `.convert_done` / `.convert_flags.jsonl` / `.convert_errors` bookkeeping) + `scripts/convert_formats.py` (per-format handlers: PDF/DOCX via docling with OCR off by default, PPTX via python-pptx with `## Slide N` anchors + speaker notes, HTML via markdownify, EML/MSG header-aware parsing; spreadsheets policy-skipped and routed to the form-catalogue skill). Skill: `.claude/skills/corpus-convert/` + `/corpus-convert` command.
- **Conversion QA / finishing layer** — `scripts/convert-docx.py`, `scripts/check-numbering.py`, `scripts/fix-numbering.py`, `scripts/docx-to-pdf.ps1`, and `bin/review-conversion.sh` for reviewing and repairing conversions before they are indexed.
- **`scripts/corpus-mcp-server.py`** — dependency-free MCP server exposing `search_corpus` and `read_document` (path-confined to `.sources/`) so Claude Desktop can query the corpus.
- **`docs/corpus-query.md`** — architecture doc: two-tier design, pipeline stages, rebind contract, local-patch table, MCP server, privacy stance.
- `tests/test_corpus_layer.py` — hermetic suite: rebind-contract tripwire, tier mapping ("Tier N" extraction + slug fallback), bonus configuration, combined status mapping. Wired into `make test` + `make test-corpus`.
- `tests/test_convert.py` — hermetic suite: numbering heuristic, EML parsing, batch bookkeeping/resume, PPTX/HTML handlers (skip cleanly when the engine is absent). Wired into `make test` + `make test-convert`. `make test` now runs **17 hermetic suites** (was 15).
- **`make dist`** — git archive of tracked files at HEAD → `brain-wiki-dist.zip`; the clean way to publish a distribution without leaking untracked local state.
- **`skills/transcript-distill`** — new skill: distill noisy ASR meeting transcripts (voice-recorder, Otter, Whisper exports) into structured wiki meeting pages (decisions, action items, topics, evidence-based speaker inference) instead of feeding raw transcripts through `wiki-ingest`. Ships `templates/meeting-template.md` with provenance frontmatter (`transcript_quality: raw-asr | polished`) and an ASR-caution callout.
- **`scripts/transcript-preprocess.py`** — deterministic, stdlib-only front half of transcript-distill: parses timestamped speaker-segment transcripts (diarized `Speaker N:` and undiarized), flags hallucination-suspect segments (builtin multilingual phrase list + exact-duplicate runs + empty segments; extensible via `--phrases`), merges consecutive same-speaker fragments into readable turns (char-capped so undiarized transcripts stay paragraph-addressable), profiles script mix (latin/hangul/cjk) and Spanish likelihood for code-switched corpora, and audits filename-date-vs-`recorded:`-metadata disagreements (`check-dates`, exit 1 on mismatch). Never modifies `.raw/` sources.
- `tests/test_transcript_preprocess.py` — hermetic suite (50+ assertions): parsing (incl. H:MM:SS timestamps, continuation lines, colon-in-prose disambiguation, trailing empty-text labels), flagging, merge barriers (flags, gaps, char cap), clean/stats/check-dates CLI round-trips, source-immutability. Wired into `make test` + `make test-transcript`.
- **Tiered ingestion depth** (`scripts/ingest-depth.py` + `wiki-ingest` §Ingestion Depth) — per-file ingestion plan (`.raw/.ingest-plan.json`) assigning each source `full | summary | catalogue | skip`, so an 850-page reference book gets a summary-plus-chapter-index page instead of full-fidelity ingestion, spreadsheets route to cataloguing, and OS junk is skipped. Deterministic heuristics (extension class + size threshold, `--large-bytes` configurable) with reasons recorded per entry; operator overrides (`set`) carry `override: true` and survive re-assignment. Feature-detected by `wiki-ingest`: no plan file → everything `full`, identical to prior behavior.
- `tests/test_ingest_depth.py` — hermetic suite: heuristic table, threshold parameterization, CLI round-trip, override survival across re-assign, dry-run non-persistence, corrupt-plan failure. Wired into `make test` + `make test-depth`.
- **`skills/form-catalogue`** — new skill: catalogue spreadsheets and survey forms into a queryable registry (`wiki/meta/form-registry.md` + form-family pages) instead of ingesting them as prose. XLSForm-aware (ODK/Kobo `survey`+`choices` convention); includes a microdata guard — workbooks that look like individual-level case data get path/dimensions/purpose entries only, flagged `contains: microdata (not summarized)`. Implements the `catalogue` depth of the ingestion plan.
- **`scripts/form-catalogue.py`** — stdlib-only workbook profiler (zipfile + ElementTree; no openpyxl/pandas): XLSForm question counts by type, group/repeat structure, choice lists with option counts, declared languages (original casing), `settings` extraction; generic-workbook and CSV/TSV per-sheet profiling; graceful `unsupported-legacy` for binary `.xls` and per-file `error` entries that never kill a batch. `registry` subcommand renders a deterministic markdown table (+ `--details` blocks). Read-only — never writes into the vault.
- `tests/test_form_catalogue.py` — hermetic suite with a synthetic in-test xlsx fixture (hand-written sheet XML): column-ref math, shared/inline/rich-run strings, sparse-row padding, XLSForm profiling, workbook/CSV profiling, legacy/corrupt/missing handling, registry rendering. Wired into `make test` + `make test-forms`.
- **Version-lineage convention** — `supersedes:` / `superseded_by:` frontmatter (single wikilink or list) modeling document supersession: policy revisions, framework editions, `LEGACY_` exports. A page without `superseded_by` is its chain's current head. Wired through three skills: `wiki-ingest` step 12 sets the symmetric pair (+ `[!stale]` callout on the old page) when ingesting a revision; `wiki-lint` check 11 runs the deterministic validator with a severity mapping (cycle/dangling HIGH, asymmetry MEDIUM auto-fixable); `wiki-query` follows `superseded_by` to the chain head before answering, citing both when history matters.
- **`scripts/lineage-check.py`** — read-only lineage validator: dangling targets, asymmetric pairs (with the exact missing-field fix), supersession cycles (DFS), and a `chains` view (heads first; forks appear as multiple heads, which is legal). Minimal stdlib frontmatter parser — no YAML dependency.
- `tests/test_lineage_check.py` — hermetic suite: frontmatter/wikilink parsing (scalar, quoted, inline + block lists, aliases), dangling/asymmetry/cycle detection, chain and fork construction, CLI exit codes (1 on findings, 2 on bad root) and JSON output. Wired into `make test` + `make test-lineage`.
- **`skills/glossary-seed`** — new skill: seed entity stubs from an organization's acronym/term table (CSV/TSV/XLSX) *before* bulk ingestion, so every later page links into a stable jargon graph instead of inventing entity names ad hoc. Stubs are thin by design (`glossary_seed: true`); `wiki-ingest` expands each the first time a source discusses it.
- **`scripts/glossary-seed.py`** — column auto-detection by header (overridable by name or index), stub + alphabetical `glossary.md` index generation, duplicate-term collapse, filename sanitization with the original term as link alias, dry-run, and a hard idempotency guarantee: existing pages are never overwritten, so expanded stubs survive re-seeding and table updates only add the delta. Reuses form-catalogue's stdlib xlsx reader.
- **Contradiction register** — `wiki/meta/contradictions.md` convention (page-pair, claim-vs-claim, found date, open/resolved status) documented in `wiki-ingest` §Contradictions; new `wiki-lint` check 12 audits callouts↔register drift and aging open contradictions, and check 13 exempts glossary stubs from orphan findings while flagging linked-but-never-expanded stubs as ingestion debt.
- `tests/test_glossary_seed.py` — hermetic suite: detection/overrides, csv + synthetic-xlsx input, sanitization, dedup, index alias links, idempotency (expanded stub survives re-run), dry-run non-persistence, usage errors. Wired into `make test` + `make test-glossary`.
- **Work queue (reground loop)** — `scripts/work-queue.py` + `wiki/meta/work-queue.md`: lint findings an ingest session can fix (stale claims, lineage gaps, aging contradictions, unexpanded stubs) become queue entries instead of dying in dated reports; ingest sessions drain the queue before new material (`wiki-ingest` §Work Queue, `wiki-lint` emission mapping). Idempotent `add` per (kind, page); checkbox edits made directly in Obsidian survive reload. Pattern adapted from llm-wiki-newsroom (MIT; see ATTRIBUTION.md).
- `tests/test_work_queue.py` — hermetic suite: entry-regex round-trip, add idempotency with detail refresh, kind-filtered done, unmatched-done exit 1, kind validation, Obsidian check-off survival, empty-queue rendering. Wired into `make test` + `make test-queue`.
- **Defect log & meta loop** (`wiki-lint` §Defect Log & Meta Loop) — bad pages get a defect row (date, skill, page, defect, root-cause tag) in `wiki/meta/defect-log.md`; when 3+ defects share a root-cause tag, lint drafts a guideline-amendment proposal into `wiki/meta/proposals/` and queues it. **The loop proposes; it never self-adopts** — no SKILL.md/template/convention change without explicit operator approval. Gives each adopting institution a mechanism to tune the toolkit to its own corpus. Pattern adapted from llm-wiki-newsroom.
- **Inbox** (`wiki-ingest` §Inbox) — `.raw/_inbox.md` drop file for URLs/paths captured mid-workday; drained at the start of every batch ingest, each line moved to a dated Processed section with a wikilink to the resulting page (failures marked, inbox always drains to empty). Pattern adapted from llm-wiki-newsroom.
- **Verifier close-out** (`wiki-ingest` §Batch Ingest step 5) — batches of 10+ pages get a pre-report pass by the existing verifier agent: the agent that wrote a page never reviews it. BLOCKER/HIGH fixed in-session; MEDIUM/LOW routed to the defect log and work queue.
- ATTRIBUTION.md — new llm-wiki-newsroom section crediting the reground/meta-loop/inbox patterns (independent implementations, MIT source, no code copied).

### Changed

- **Project renamed: claude-obsidian → brain-wiki** for standalone publication under [`chulsooiam/brain-wiki`](https://github.com/chulsooiam/brain-wiki). brain-wiki is a derived work of [`AgriciDaniel/claude-obsidian`](https://github.com/AgriciDaniel/claude-obsidian) v1.9.2 (MIT); the original copyright notice is retained in LICENSE and the credit chain lives in ATTRIBUTION.md. Plugin id, marketplace slug, dist artifact, and all in-repo references renamed; upstream-specific community/funding material removed.
- **Hybrid retrieval is now default-on.** Both retrievers (`scripts/retrieve.py` and `scripts/corpus-retrieve.py`) self-heal a missing or stale index inline — synthetic prefixes only, zero egress — announcing the rebuild with an `auto-build:` stderr note; `--no-auto-build` restores the old exit-10 refusal. `bin/setup-retrieve.sh` is no longer a required provisioning step (it remains available for manual pre-provisioning and the egress consent flow). The only remaining opt-in is egress: LLM-generated contextual prefixes. `tests/test_retrieve.py` gained default-on self-heal contract tests.
- **Distribution scrub: generic seed vault.** The tracked wiki is now a generic scaffold — `wiki/overview.md`, `index.md`, `hot.md`, `log.md`, per-category `_index.md` files, and the `wiki/references/` toolkit pages — with no personal or instance-specific content in tracked files.
- `.claude-plugin/plugin.json` + `marketplace.json` version 1.9.2 → 2.0.0; `CITATION.cff` version + release date updated.

### Fixed

- **Unanchored `WIKI*.md` gitignore rule** — the pattern matched anywhere in the tree and (case-insensitively) was silently excluding four toolkit files from the repository: `.claude/commands/wiki-query.md`, `agents/wiki-ingest.md`, `agents/wiki-lint.md`, and `commands/wiki.md`. The rule is now root-anchored (`/WIKI*.md`); all four files ship.
- **Dead `CORPUS_QUERY.md` references** — all pointers now target the doc's real home, `docs/corpus-query.md`.
- **Drift-detector hook no longer ships in tracked settings** — moved out of the tracked settings file into `settings.local.json` (per-machine, untracked), so a fresh clone does not inherit a deployment-specific hook.
- **`.vault-meta/locks/.gitkeep` restored** — the lock directory exists again on fresh clone, matching the v1.9.1 gitignore contract.
- **`allocate-address.sh` was flock-only and unusable on Git Bash/Windows** — every allocation died with `flock: command not found`, taking DragonScale addressing down entirely on such hosts. Now mirrors `wiki-lock.sh`: flock when available, portable mkdir-based lock otherwise (15s timeout in the fallback — Windows process spawn is slow enough that 10 queued writers legitimately need >5s; 60s staleness break). The stale-age check also validates `stat` output as a pure integer: GNU stat treats the BSD-style `-f %m` fallback as filesystem mode and emits a multi-line report, which previously poisoned the arithmetic and killed waiting allocators mid-retry (observed: 10 concurrent allocations, 7 survivors).
- **Dead `wiki/references/` links from 8 skills** — `transport-fallback.md` and `methodology-modes.md` are toolkit documentation that lived under `wiki/` and were lost in a content reset; restored. Packaging note: these two ship with the toolkit even though they live in the wiki tree.
- **Instance-specific text scrubbed for distribution** — hardcoded absolute vault paths and deployment-specific archive references removed from `CLAUDE.md` and `scripts/corpus-mcp-server.py` (server renamed from an instance-specific name to `vault-corpus`, examples genericized, corpus counts unpinned).
- `tests/test_wiki_lock.sh` — the carriage-return rejection case now probes whether the platform delivers CR bytes through piped argv (MSYS/Git Bash strips them, so the CR never reached the script and the case mis-failed, leaking a lock that also broke the 10-path stress count); skips with a note when CR can't be delivered. The validation itself is unchanged and correct.
- **Windows clock-resolution collision in Zettelkasten ID minting** (`scripts/wiki-mode.py`). `mint_zettel_id()` relied on `datetime.now()` microsecond resolution for uniqueness; Windows' wall clock can tick coarser, and 10 rapid calls produced 10 identical IDs (caught by the existing `test_wiki_mode` distinctness test on a Windows host). A module-level monotonic guard now advances one microsecond past the last issued ID whenever the clock hasn't moved — IDs strictly increase within a process on every platform.

### Verification

- Validated against a real 187-transcript corpus (24,273 segments, 9 min-4h44m meetings, EN/ES/KO content): 0 parse failures after the undiarized-format fix, 10 hallucination segments flagged, 1 Spanish-dominant file detected, and — decisively for the `check-dates` design — **107 of 187 filename dates disagreed with the recording metadata**.
- Depth heuristics validated against a real 404-file institutional corpus (~800 MB): 294 full / 41 summary (23 slide decks + 18 large documents) / 67 catalogue (spreadsheets) / 2 skip (`desktop.ini`) — matching an independent manual corpus analysis.
- Form profiler validated against a real 73-file survey-form folder: 0 crashes; 34 XLSForms detected (4,109 questions; languages incl. English, French, Arabic, Bengali, Somali, Kreyòl), 16 generic workbooks, 3 legacy `.xls` flagged, 20 non-spreadsheet files correctly declined.
- Glossary seeder validated against a real 296-row institutional acronym workbook: 284 unique terms auto-detected (columns found by header with no overrides), duplicates collapsed, dry-run clean.
...
5 of 7 axes #1 (unchanged count, but methodology axis deepens — brain-wiki is now the only Claude+Obsidian plugin shipping methodology modes AND a first-class thinking-loop framework AND per-skill thinking guidance).
