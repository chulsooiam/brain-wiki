...
#
# flock(1) when available (Linux, macOS with util-linux). Git Bash on Windows
# ships no flock, so fall back to a portable mkdir-based lock — mkdir is atomic
# on every platform. Same 5s timeout; a stale lock older than 60s is broken
# (crashed holder). Mirrors the wiki-lock.sh meta-lock fallback.
...
if command -v flock >/dev/null 2>&1; then
  ...
  while ! mkdir "$LOCK_DIR" 2>/dev/null; do
    now="$(date +%s)"
    # GNU stat: -c %Y. BSD/macOS stat: -f %m. On GNU, `-f` means filesystem
    # mode and emits a multi-line report, so the fallback's output must be
    # validated as a pure integer — anything else (including the lock dir
    # vanishing mid-check) counts as "fresh" and just retries.
    ...
    tries=$((tries + 1))
    # 15s (vs flock's 5s): on Windows, bash process spawn is slow enough
    # that 10 queued writers can legitimately need >5s to drain.
    if [ "$tries" -ge 150 ]; then
      echo "ERR: could not acquire address allocator lock within 15s" >&2
      exit 1
    fi
    sleep 0.1
  done
