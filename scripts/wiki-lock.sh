...
# Portable mkdir-based meta-lock, used when flock(1) is unavailable.
#
# Git Bash on Windows ships no util-linux flock, so the flock path below dies
# with "command not found" on every command — which the PostToolUse hook reads
# as a lock-check failure and silently defers auto-commit forever. mkdir is
# atomic on POSIX and NTFS alike, so it gives the same mutual exclusion.
#
# Same 5s timeout as `flock -w 5`. Staleness escape (dead owner PID, or age
# past STALE_AFTER_SEC) so a crashed holder can't wedge the vault; a lock whose
# epoch file isn't written yet is treated as live, not stale, so the reaper
# can't break a mutex that another process is mid-way through taking.
_meta_lock_dir_acquire() {
  local waited=0 owner epoch age
  while ! mkdir "$META_LOCK_DIR" 2>/dev/null; do
    owner=$(cat "${META_LOCK_DIR}/pid" 2>/dev/null || true)
    epoch=$(cat "${META_LOCK_DIR}/epoch" 2>/dev/null || true)
    age=0
    case "$epoch" in
      ''|*[!0-9]*) : ;;
      *) age=$(( $(now_epoch) - epoch )) ;;
    esac
    if [ -n "$epoch" ] && { { [ -n "$owner" ] && ! is_alive "$owner"; } \
                            || [ "$age" -ge "$STALE_AFTER_SEC" ]; }; then
      rm -rf "$META_LOCK_DIR" 2>/dev/null || true
      continue
    fi
    [ "$waited" -ge 25 ] && return 1
    sleep 0.2
    waited=$(( waited + 1 ))
  done
  printf '%s' "$$" > "${META_LOCK_DIR}/pid" 2>/dev/null || true
  now_epoch > "${META_LOCK_DIR}/epoch" 2>/dev/null || true
  return 0
}
...
with_meta_lock() {
  ...
  if command -v flock >/dev/null 2>&1; then
    ...
    _meta_lock_dir_acquire || die "could not acquire meta-lock within 5s" 1
    local rc=0
    # Subshell so this branch matches the flock branch: option changes and a
    # die() inside the command can't escape, and the mutex is always released.
    ( "$@" ) || rc=$?
    rm -rf "$META_LOCK_DIR" 2>/dev/null || true
    return "$rc"
  fi
