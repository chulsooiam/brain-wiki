...
# fcntl is POSIX-only; Windows gets the equivalent single-byte lock via msvcrt.
# Both give an exclusive, non-blocking, advisory lock released on close, which
# is all the index write needs.
if os.name == "nt":
    import msvcrt
...
# LOCAL PATCH (2026-07-28): chunk ids ("<address>:<idx>") to leave out of the
# index. Empty here, so the wiki tier is unaffected; corpus-bm25.py rebinds it
# from the dedup manifest. Filtering at build time lets corpus-dedup.py mark
# duplicates instead of moving their files out of chunks/, which kept the
# chunk store complete — see docs/corpus-query.md.
