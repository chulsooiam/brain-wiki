...
def mint_zettel_id():
    ...
    Monotonic guard: Windows' wall clock can tick coarser than a microsecond,
    so back-to-back calls may read an identical timestamp. If the clock has
    not advanced past the last issued ID, advance one microsecond past it —
    IDs are strictly increasing within a process regardless of platform
    clock resolution.
    """
    global _last_minted
    now = datetime.now(timezone.utc)
    if _last_minted is not None and now <= _last_minted:
